    ...
    <section id="pricing" className="section-light">
      <div className="max-w-[1280px] mx-auto px-6 py-24 lg:py-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-12 max-w-[860px]">
          {PRICING.map((p) => (
            <div key={p.tag} className={`rounded-2xl p-7 ${p.highlight ? 'bg-slate-900 text-white border border-slate-800 shadow-[0_30px_60px_-30px_rgba(15,23,42,0.4)]' : 'card-soft'}`}>
              <div className={`text-[11px] tracking-[0.14em] uppercase font-semibold ${p.highlight ? 'text-violet-300' : 'text-violet-600'}`}>{p.tag}</div>
              ...
              <a href="#signup" className={`mt-6 inline-flex items-center justify-center w-full py-2.5 rounded-lg text-[14px] font-medium transition-colors ${p.highlight ? 'bg-violet-500 hover:bg-violet-400 text-white' : 'btn-violet'}`}>{p.cta}</a>
            </div>
          ))}
        </div>
      </div>
