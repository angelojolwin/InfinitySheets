          ...
          {TESTIMONIALS.map((t, i) => (
            <div key={i} className="card-soft p-6">
              ...
              <div className="mt-6 pt-5 border-t border-[color:var(--color-border)] flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-200 to-blue-200 flex items-center justify-center text-[12px] font-semibold text-violet-700">{t.name.split(' ').map((s) => s[0]).slice(0, 2).join('')}</div>
